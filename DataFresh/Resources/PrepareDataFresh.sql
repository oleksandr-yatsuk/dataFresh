...
AS
	...
	BEGIN
			...
			SET @sql = N'IF(SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE table_name = ''' + @Table + ''' AND table_schema = ''' + @Schema + ''' AND IDENT_SEED(TABLE_NAME) IS NOT NULL) > 0
			BEGIN				
...
AS
	...
	DECLARE UserTable_Cursor CURSOR FOR
		SELECT [tablename], [tableschema] FROM #UserTables WHERE tablename not in ('df_ChangeTracking', 'dr_DeltaVersion')
